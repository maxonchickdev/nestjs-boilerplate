...
import { NestjsBoilerplateMobileAdmin } from './nestjs-boilerplate-admin.module';
...
(async (): Promise<void> => {
	const app = await NestFactory.create<NestExpressApplication>(NestjsBoilerplateMobileAdmin);
	...
	logger.log(`Nestjs boilerplate mobile application is running on: ${await app.getUrl()}`);

	if (!isProduction) logger.log(`Swagger docs available at: ${await app.getUrl()}/api/docs`);
})().catch(e => {
	logger.error(`Faild to start nestjs boilerplate mobile application: ${e}`);
});
